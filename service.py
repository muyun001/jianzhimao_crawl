...
class MysqlService(object):
    ...
    def create_table():
        ...
        """
        ct_sql = f"""create table if not exists `{config.CITY_TABLE}`(
                                ...
                                primary key (`id`))"""
        rg_sql = f"""create table if not exists `{config.REGION_TABLE}`(
                                ...
                                `crawl_time` datetime not null default now() comment "抓取时间",
                                primary key (`id`))"""
        jb_sql = f"""create table if not exists `{config.JOB_TABLE}`(
    ...
    def insert_city(ch_dict):
        ...
        for city, href in ch_dict.items():
            insert_sql = f"""insert ignore into 
                            {config.CITY_TABLE}(`id`, `city`,`city_href`) 
                            values 
...
class CrawlService(object):
    ...
    def crawl_regions(self, href):
        ...
        if response.status_code != config.RESPONSE_STATUS_CODE_NORMAL:
