    ...
    while True:
        ...
        for r_c in region_tuple:
            ...
            try:
                ...
                # region_job_list是某地区的所有11页兼职列表的数据，[{},{},{}]
            ...
            # 抓取详情页
            for job_list_dict in region_job_list:
                """
                ...
                """
                job_detail_dict = crawl_service.crawl_job_detail(job_list_dict["url"])
                if job_detail_dict == config.FUNC_CODE_ERROR:
                    logging.info(f"抓取{region}(url:{job_list_dict['url']})的兼职列表失败！")
                    continue

                job_dict = {**job_list_dict, **job_detail_dict}  # 将列表页数据与详情页数据合二为一

                # 插入数据库
                if MysqlService.insert_job(region_id, job_dict) == config.FUNC_CODE_ERROR:
                    logging.error("插入数据失败！", job_list_dict)

            logging.info(f"修改{region}(id:{region_id})的状态码为'抓取完成'")
            if MysqlService.update_status_code(f"('{region_id}')", status_code=config.STATUS_CRAWL_SUCCEED,
